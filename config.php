...
class lashopak
{
    ...
    public function addCart($memberID, $prodID, $nama, $harga, $jumlah, $total, $note)
    {
        ...
        if ($count == 0) {
            $query = "INSERT INTO cart(memberID, prodID, nama, harga, jumlah, total, note) VALUES (:memberID, :prodID, :nama, :harga, :jumlah, :total, :note)";
            $stmt = $this->db->prepare($query);
            $stmt->execute(array(':memberID' => $memberID, ':prodID' => $prodID, ':nama' => $nama, ':harga' => $harga, ':jumlah' => $jumlah, ':total' => $total, ':note' => $note));
            if (!$stmt) {
                return "Gagal";
            }else{
                return "Sukses";
